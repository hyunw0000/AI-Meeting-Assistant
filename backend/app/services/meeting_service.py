...
규칙:
- 설명 문장 없이 JSON만 반환해.
- summary는 다음 항목 순서대로 작성하고, 각 항목 앞에 줄바꿈(\n)을 넣어.
- 형식:
  📌 주제: (회의 텍스트에 나온 회의 제목)
  👥 참여자: (회의 텍스트에 나온 참석자 이름만 쉼표로 나열)
  📋 안건: (회의 텍스트에 나온 안건만 쉼표로 나열)
  ✅ 주요 논의:
  - (참석자 이름): (담당 작업) ((마감일))
  🗓️ 다음 회의: (회의 텍스트에 나온 다음 회의 일정)
- 각 항목 앞에 이모지를 그대로 붙여서 사용해.
- 회의 텍스트에 명시된 내용만 사용하고, 추측해서 만들지 마.
- 회의 텍스트에 없는 항목은 아예 생략해. 빈 칸으로 두거나 "미정"으로 채우지 마.
- 괄호 안 설명은 실제 내용으로 채워. 괄호와 설명 문구는 출력하지 마.
