...
const fetchDbCalendarEvents = async () => {
  ...
  const uploadMeetingAudio = async (file: File) => {
    try {
      ...
      const newMeeting: Meeting = {
        ...
        title: savedMeeting.meeting_result?.title || '회의록',
      ...
      <div className="tile-events">
        {dayMeetings.slice(0, 3).map(m => (
          <div
            ...
            //마감일정 눌렀을 때 기존 회의록 뜨게하는거
            /*onClick={e => {
              e.stopPropagation()
              const targetId = m.id.startsWith('task_')
                ? m.id.split('_')[1]
                : m.id
    ...
    >
            ...
            >
              {isUploading ? (
                ...
                <>
                  ...
                  <p>음성 파일을 선택하세요</p>
                  <p className="hint">
                    선택 날짜: {formatLocalDate(selectedDate ?? new Date())}
                  </p>
        ...
        <section className="main-calendar-section">
          <div className="calendar-card-large">
            ...
            <Calendar
              value={selectedDate}
              onClickDay={(date, e) =>
                handleTileClick(date, e as unknown as React.MouseEvent)
              }
      ...
      {createPopup && (
        <div
          ...
          style={{
            top: createPopup.y,
            left: Math.min(createPopup.x, window.innerWidth - 340),
          }}
          onClick={e => e.stopPropagation()}
        >
          <div className="create-popup-header">
            <span className="create-popup-date">
              {createPopup.date.toLocaleDateString('ko-KR', {
                month: 'long',
                day: 'numeric',
              })}{' '}
              새 회의
  ...
  <div
    ...
    onClick={() => setListModalDate(null)}
  >
    <div
      className="modal-content card list-modal"
      onClick={e => e.stopPropagation()}
    >
      <div className="detail-header">
        <div className="title-with-icon">
          <h2>
            {listModalDate.toLocaleDateString('ko-KR', {
              month: 'long',
              day: 'numeric',
            })}
            의 회의
